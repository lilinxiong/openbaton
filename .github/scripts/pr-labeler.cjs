...
function detectTypeLabelFromTitle(title) {
  ...
  // Sentence-case fallback (e.g. "Fix Codex adapter skill install").
 ...
 *
 * Titles that are not conventional (`fix(scope): …`) and not a leading
 * sentence-case type (`Fix …`) fall through. Conventional commit subjects
 * under the PR can still classify it.
 *
 * `chore` is supporting, not competing. `test:`, `ci:`, `chore:`, `style:`,
 * `refactor:`, and `build:` all map to it. If those sit next to one real type
 * (`fix:` plus `test:`), keep the real type. Mixed real types (`fix:` and
 * `feat:`) stay unlabeled rather than guessed.
 */
