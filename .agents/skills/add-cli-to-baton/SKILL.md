...
All hosts are hookless. The director owns ticket presence, orchestration, and safety through explicit command-boundary checks, Receipts, native execution handles, and Git audits.
...
5. Preserve host-scoped configuration. `cli.<target>.enabled` controls Baton only in that host; never consult another profile, create unselected placeholder profiles, or invent a global default CLI. A disabled or missing target profile must fail closed and never borrow another host's profile or model. Director max_concurrent/max_depth remain independent fallbacks; persist a target override only when the CLI explicitly reports it. Runtime commands must pass `--host <target>` (or resolve the host from `BATON_HOST` / a unique runtime signal).
