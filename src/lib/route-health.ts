...
export function readRouteHealth(cwd: string): RouteHealthStore {
  const file = routeHealthPath(cwd);
...
function writeRouteHealth(cwd: string, store: RouteHealthStore): void {
  const file = routeHealthPath(cwd);
...
export function cardsForAutomaticSelection(cwd: string, cards: ModelCard[], taskText: string, host = "codex"): ModelCard[] {
  return cards.filter((card) => isCardAutoEligible(cwd, card, taskText, { host }));
}
