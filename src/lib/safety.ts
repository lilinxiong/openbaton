...
function stagedIndexControlMetadata(baseline: CommitBaseline): {
  index_control_algorithm?: unknown;
  index_control_checksum?: unknown;
  index_control_entry_count?: unknown;
} {
...
function auditPreparedCommitRules(root: string, obs: PreparedCommitObservation, baseline: CommitBaseline): CommitSafetyVerdict {
...
function auditCommitOutcomeRules(
  root: string,
...
function auditWorktreeRules(root: string, obs: WorktreeAuditObservation, baseline: GitBaseline, policy: SafetyPolicy): SafetyVerdict {
...
function observePreparedCommitSync(root: string): PreparedCommitObservation {
...
function observeCommitOutcomeSync(root: string): CommitOutcomeObservation {
...
function observeWorktreeAuditSync(root: string): WorktreeAuditObservation {
  const reflog = headReflog(root);
...
function preparedObservationFromFacts(facts: StableGitSafetyFacts): PreparedCommitObservation {
  return {
    head: facts.head,
...
function commitOutcomeObservationFromFacts(facts: StableGitSafetyFacts): CommitOutcomeObservation {
...
function worktreeObservationFromFacts(facts: StableGitSafetyFacts): WorktreeAuditObservation {
  return {
    head: facts.head,
    branch: facts.branch,
    branchRef: facts.branchRef,
