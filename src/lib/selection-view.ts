...
function viewState(proposal: SelectionProposal, options: SelectionViewOptions = {}) {
  ...
  return {
    ...
    host_snapshot_id: proposal.host_snapshot_id,
    host_snapshot_ids: approvalTargets.map((target) => ({ scope: target.scope, id: target.host_snapshot_id })),
...
function bundleProposal(inputs: SelectionBundleInput[]) {
  ...
  for (const input of inputs) {
    ...
    approvalTargets.push({
      ...
      host_snapshot_id: input.proposal.host_snapshot_id,
      units: targetUnits,
    });
  }
  ...
  const proposal: SelectionProposal = {
    ...
    approved_at: null,
    host_snapshot_id: `bundle:${approvalTargets.map((target) => target.host_snapshot_id).join(",")}`,
...
export function renderSelectionView(proposal: SelectionProposal, options: SelectionViewOptions = {}): string {
  ...
  <div class="viz-grid">
    ...
    <div class="card viz-stat"><div class="text-muted">可选择候选 route/profile</div><div class="viz-stat-value" data-model-count></div><div class="text-small">严格等于当前 host ∩ OpenCodex catalog；不可调用项另行披露</div></div>
  ...
  <script>
    (() => {
      ...
      const showDetail=(candidate)=>{const pool=byPool.get(candidate.quota_pool_id);detail.innerHTML='<div class="baton-detail-grid"><div><div class="text-muted text-small">exact route/profile</div><code>'+esc(candidate.model_id)+'</code><div><span class="viz-badge">'+esc(evidence(candidate))+'</span></div><div class="text-small">优选任务 '+esc(preferred(candidate))+'</div></div><div><div class="text-muted text-small">模型擅长项</div><div>'+esc(localizedStrengths(candidate.strengths))+'</div></div><div><div class="text-muted text-small">AA 评分</div><div>智力 / 编码 / Agent '+esc(aa(candidate))+'</div><div>单任务成本 '+esc(metric(candidate.aa_scores.cost_per_task))+' · tokens/s '+esc(metric(candidate.aa_scores.output_tokens_per_second))+' · 首答延迟 '+esc(metric(candidate.aa_scores.time_to_first_answer_seconds))+'s</div><div class="text-small">'+esc(aaData(candidate))+'</div></div><div><div class="text-muted text-small">AA / reference provenance</div><div>'+esc(provenance(candidate))+'</div></div><div><div class="text-muted text-small">quota</div><div>'+esc(pool.label+' · '+poolQuota(pool))+'</div><div class="text-small">'+esc(poolDetail(pool))+'</div></div><div><div class="text-muted text-small">汇总任务评分</div><div class="viz-stat-value">'+esc(aggregate(candidate)==null?'—':aggregate(candidate).toFixed(1))+'</div></div><div><div class="text-muted text-small">callability</div><div>'+esc(candidate.host.code+' · '+hostReason(candidate))+'</div></div></div>'};
      ...
      root.querySelector("[data-snapshot]").textContent="Host snapshots "+state.host_snapshot_ids.map((item)=>item.scope+"="+item.id).join(" · ")+" · catalog "+state.catalog_fingerprint+" · confirmation "+state.confirmation_id;
