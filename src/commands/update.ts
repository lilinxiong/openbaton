...
/**
 * Refresh Baton-owned host files and user-global CLI settings.
...
export function updateProject(
  ...
  if (!fs.existsSync(destConfig)) {
    fs.copyFileSync(
      path.join(tmplRoot, "templates", "config.toml"),
      destConfig,
    );
    actions.push(
      `wrote ${displayHomePath(destConfig, { cwd, env })} (was missing)`,
    );
  } else {
    const current = loadConfig(cwd, { env });
    saveConfig(cwd, current, { env });
    actions.push(
      `kept profiles in ${displayHomePath(destConfig, { cwd, env })}`,
    );
  }
  const hosts = refreshInstalledHostSkills(cwd, { env });
  actions.push(...hosts.actions);
  writeInstallManifest(
    buildInstallManifest(cwd, hostIds(env), env, adapters.ownership),
    env,
  );
