...
2. **Configuration is per CLI and user-global.** Store each `cli.<id>` profile's enabled flag, runner label, longctx label, and subagent_models allowlist in ~/.baton/config.toml. There is no global default CLI.
