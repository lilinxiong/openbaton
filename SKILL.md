...
3. **Model disclosure and user confirmation are mandatory for ordinary delegation.** Turn one broad user request into bounded units with an objective, deliverable, and done condition. Route configured mechanical units first, independent of whether they came from `--unit` or OpenSpec, then aggregate every remaining unit into one immutable request-level proposal per workspace; never create one standalone proposal per unit. Only configured ops may skip the selector through `confirmed_by=ops-config`.
...
- Do not create, reserve, or spawn an ordinary delegated ticket before the user confirms the disclosed selection proposal. Configured mechanical ops are the sole `confirmed_by=ops-config` exception.
