...
The new Codex task must derive the current `spawn_agent` model and reasoning-effort surface itself and silently run `baton host sync` before it proposes models. Do not manually maintain a model list. OpenCodex may expose more catalog routes/profiles than the current Codex task can spawn; the proposal must show that difference.
...
The pasted business request is intentionally trigger-neutral. Codex must not ask the user to invoke Baton. It should first break down the work and present model-selection disclosure containing:

- one preferred exact route/profile when scoring has a unique positive winner, otherwise an explicit manual-choice state;
- all policy-eligible currently callable candidates, strengths, task score, raw AA intelligence/coding/agentic scores, remaining quota/reset or an explicit unknown reason, and callability;
...
- candidate disclosure covers strengths, task/AA scores, quota, callability, and unavailable providers;
