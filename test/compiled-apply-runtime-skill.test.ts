...
const commonInvariants: Array<[string, RegExp]> = [
  ...
  ["Spark progression", /(?=[\s\S]*Spark)(?=[\s\S]*(?:first candidate|first))(?=[\s\S]*(?:under-capable|exhausted))(?=[\s\S]*(?:silently|silent))(?=[\s\S]*(?:unconfigured|configured route))/i],
  ["complete no-qualified diagnostics", /(?=[\s\S]*NO_QUALIFIED_CANDIDATE)(?=[\s\S]*(?:every|each))(?=[\s\S]*(?:candidate|configured))(?=[\s\S]*(?:exclusion reason|exclusion))/i],
  ["session-local recovery", /(?=[\s\S]*session-local)(?=[\s\S]*(?:cache|facts))(?=[\s\S]*new\s+(?:[A-Za-z]+\s+)?session)(?=[\s\S]*(?:recheck|check))/i],
  ...
  ["director return conditions", /(?=[\s\S]*source staleness)(?=[\s\S]*changed contracts)(?=[\s\S]*scope changes)(?=[\s\S]*safety-blocked partial mutation)(?=[\s\S]*PLAN_INSUFFICIENT)/i],
