...
12. **Route data is local-first.** Read the persisted OpenCodex Route Snapshot plus local AA capability cache from `~/.baton/cache`. Refresh `baton routes` only when the OpenCodex catalog/config fingerprint changes or the user explicitly requests refresh. Effective callability is the intersection with the session-scoped Codex host snapshot. Missing route is blocked and missing host route is `HOST_ROUTE_UNAVAILABLE`. Exact AA evidence is preferred; deterministic base-profile or suffix-free serving-variant evidence is labelled `reference_only`, never used for automatic recommendation, and otherwise missing evidence stays `unranked`. Read quota through OpenCodex first. For an absent/unknown provider only, read the local CodexBar GUI snapshot (then history, then CLI) so fallback numbers match the CodexBar UI; never overwrite OpenCodex reported windows. Persist only sanitized percentage/reset windows and `codexbar:...` provenance, never account/auth/raw-output fields. Missing reports stay unknown. Mechanical ops use user-global `[ops.runner]` / `[ops.longctx]` in `~/.baton/config.toml`; never read or create a project `.baton.toml`. `baton config` refreshes its model snapshot directly through OpenCodex and does not depend on a host snapshot. Empty routes stay on the director; configured callable routes skip the selector (`confirmed_by=ops-config`), while dispatch still validates current-session callability. Wait for ops worker conclusions, including build/test failure. Workers never commit.
...
0. **Sync and confirm.** Extract every exact model route and allowed reasoning effort from this session's complete calling-host model selector/tool schema, run `baton host sync` with repeated `--model`/`--profile`, then create one request-level proposal per workspace. Do not substitute a shorter `spawn_agent` override hint for that host surface. Render all proposals from this front request into one selector with one global Provider choice and one Submit, then emit its returned inline content reference in the current Codex conversation only; if inline rendering is unavailable, use one consolidated grouped-text disclosure here instead. Never navigate to the artifact or link it. Wait for that Submit/user confirmation or route changes. Run each generated `baton selection approve ... --confirm` command with the shared bundle confirmation id only after that confirmation. No approval means no ticket and no spawn.
