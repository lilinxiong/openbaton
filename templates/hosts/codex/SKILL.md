...
2. **Concrete units before selection.** First refine one broad user request into bounded units with an objective, deliverable, and done condition. Resolve configured mechanical ops per unit from its own description plus the unchanged request context; `--unit` is structure, not a selector-forcing switch. Conflicting actions stay on the ordinary confirmation path, explicit `--model` disables ops-config auto-routing, and one request may create at most one commit-only unit. Aggregate all remaining units into one request-level proposal per workspace; never create one standalone proposal per unit. Only configured ops or user-approved proposals become FIFO tickets; queued is not running. The host reserves runnable tickets with `baton dispatch next --host codex --capacity <effective-host-capacity> --json` and executes each reserved spec itself.
...
0. **Sync and confirm.** After OpenCodex has synchronized its runtime/provider state, run `baton routes refresh` once only when Baton's snapshot is missing/stale or refresh was explicitly requested; never inspect or publish a per-session model surface. Resolve and disclose configured ops first, then create one request-level proposal per workspace for the remaining units. Render all proposals from this front request into one selector with one global Provider choice and one Submit, then emit its returned inline content reference in the current Codex conversation only; if inline rendering is unavailable, use one consolidated grouped-text disclosure here instead. Never navigate to the artifact or link it. Wait for that Submit/user confirmation or route changes. Run each generated `baton selection approve ... --confirm` command with the shared bundle confirmation id only after that confirmation. Without approval, no ticket or spawn may be created for selector-bound units; configured ops remain the sole `confirmed_by=ops-config` exception.
...
- Do not create or dispatch an ordinary delegated ticket before the user confirms the disclosed model proposal. Configured mechanical ops are the sole `confirmed_by=ops-config` exception.
