...
4. **Disclose once and confirm once.** Provider is one global multi-select. Below it, the director shows every candidate and all task assignments together. Multiple workspace proposals from the same front request are rendered as one bundle with one Submit. Units shown in that selector cannot create tickets or subagents before Submit; separately disclosed configured ops may already be queued or running through `confirmed_by=ops-config`.
...
`spawn` resolves ops per work unit, including units supplied with `--unit`. A unit-specific action and an unambiguous request-level action may provide each other's context; conflicting actions stay behind ordinary model selection. An explicit `--model` always disables ops-config auto-routing. Mixed requests may dispatch their configured mechanical units while keeping the remaining units in one selector. A single request may create at most one `commit-only` unit.
